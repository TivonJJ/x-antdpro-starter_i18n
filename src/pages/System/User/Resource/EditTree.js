...
export default class extends React.Component {
    ...
    del=(permission)=>{
        ...
        Modal.confirm({
            ...
            content:<div style={{lineHeight:1.8}} dangerouslySetInnerHTML={
    ...
    render(){
        ...
        const loop = data => data.map((item) => {
            ...
            if(selectedKeys[0] === item.dna){
                after = <div className="menu-item-bars inline">
            ...
            if(filter){
                ...
                name = index > -1 ? <span>{beforeStr}<span style={{ color: '#f50' }}>{filter}</span>{afterStr}</span>
                    :<span>{name}</span>;
            }
            const title = <span className={classNames({'text-primary':!isExists})}>{before}{name}{after}</span>;
        ...
        return <div>
            {permissions.length<=0&&<div className="gutter-left-lg gutter-top">
